...
// TCP can short-write, loop until everything is sent
...
// Build frame with BE16 size header
static std::vector<uint8_t> build_frame_be(uint8_t type, uint8_t seq, const std::vector<uint8_t>& payload) {
    const size_t total = HEADER_SIZE + payload.size();
    if (total > 0xFFFF) throw std::runtime_error("frame too big");
...
// Fixed-size ASCIIZ login payload from views
static std::vector<uint8_t> build_login_payload(std::string_view user, std::string_view pass) {
    std::vector<uint8_t> p(USER_PASS_FIELD * 2, 0);
    const size_t ulen = std::min<size_t>(user.size(), USER_PASS_FIELD - 1);
    const size_t plen = std::min<size_t>(pass.size(), USER_PASS_FIELD - 1);
...
static bool read_frame(socket_t s, uint8_t& type, uint8_t& seq, std::vector<uint8_t>& payload) {
    ...
    if (!recv_all(s, hdr, HEADER_SIZE)) return false;

    const uint16_t msg_size = (uint16_t(hdr[0]) << 8) | uint16_t(hdr[1]);
    if (msg_size < HEADER_SIZE) return false;

    type = hdr[2];
    seq  = hdr[3];

    const size_t payload_len = msg_size - HEADER_SIZE;
...
int main(int argc, char** argv) {
    ...
    // --- 1) Login ----
    const auto login_payload = build_login_payload(user, pass);
    const auto login_frame   = build_frame_be(MSG_LOGIN_REQ, seq_login, login_payload);
    ...
    if (!cipher.empty()) {
        xor_with_lcg_inplace(seed, cipher.data(), cipher.size()); // encrypt in place
    }
    ...
    const auto echo_frame = build_frame_be(MSG_ECHO_REQ, seq_echo_u8, echo_payload);
