...
void print_frame(const Frame &f) {
    std::cout << "Frame size=" << f.message_size << " type=" << int(f.message_type)
              << " seq=" << int(f.message_sequence) << " payload_len=" << f.payload.size() << "\n";
}
...
int main() {
    ...
    std::vector<uint8_t> part1(frame1.begin(), frame1.begin() + 2); // first two bytes (size hi/lo)
    ...
    // Test 4: malformed size (too small)
    try {
        std::vector<uint8_t> bad = {0x00, 0x02, 0x11, 0x22}; // message_size = 2 (<4) -> invalid
        p.feed(bad);
